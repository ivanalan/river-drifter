...
void initPlayer()
{
    ...
    shadowOAM[0].attr2 = ATTR2_TILEID(player.id, player.aniState);
}
...
void updateTimeline()
{
    ...
    shadowOAM[3].attr1 = (65 + (player.distanceTraveled / speed)) | ATTR1_TINY;
    if (cheat)
    {
        shadowOAM[3].attr2 = ATTR2_TILEID(20, 0);
    }
    else
    {
        shadowOAM[3].attr2 = ATTR2_TILEID(player.id, 0);
    }

    int interval = 70;
    //left bracket
    shadowOAM[10].attr0 = 15 | ATTR0_SQUARE;
    ...
    if ((65 + (player.distanceTraveled / speed)) == interval)
    {
        endGame = 1;
    }
}
...
void updatePlayer()
{
    ...
    if (cheat == 1)
    {
        shadowOAM[0].attr2 = ATTR2_TILEID(20, player.aniState);
    }
    else
    {
        shadowOAM[0].attr2 = ATTR2_TILEID(player.id, player.aniState);
